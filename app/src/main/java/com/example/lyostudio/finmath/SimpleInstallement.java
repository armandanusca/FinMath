...
public class SimpleInstallement extends AppCompatActivity
            ...
            public void onClick(View view) {
                ...
                if(cnt>1){
                    ...
                    if (poz == 4) {
                        ...
                        float mon = Float.parseFloat(months.getText().toString());
                        float sumfin = initsum * (1 + (mon / 12) * (perc / 100));
                        fin_sum.setText(Float.toString(sumfin));
                    } else if (poz == 3) {
                        float initsum = Float.parseFloat(init_sum.getText().toString());
                        float perc = Float.parseFloat(percent.getText().toString());
                        float sumfin = Float.parseFloat(fin_sum.getText().toString());
                        float mon = (sumfin / initsum - 1) / (perc / 100) * 12;
                        months.setText(Float.toString(mon));
                    } else if (poz == 2) {
                        float initsum = Float.parseFloat(init_sum.getText().toString());
                        float sumfin = Float.parseFloat(fin_sum.getText().toString());
                        float mon = Float.parseFloat(months.getText().toString());
                        float perc = (sumfin / initsum - 1) / (mon / 12) * 100;
                        percent.setText(Float.toString(perc));
                    } else if (poz == 1) {
                        float perc = Float.parseFloat(percent.getText().toString());
                        float mon = Float.parseFloat(months.getText().toString());
                        float sumfin = Float.parseFloat(fin_sum.getText().toString());
                        float initsum = sumfin / (1 + (mon / 12) * (perc / 100));
                        init_sum.setText(Float.toString(initsum));
                    }
                }
            }
        });
        definition = (Button) findViewById(R.id.def_sist);

        definition.setOnClickListener(new View.OnClickListener()
        {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(getApplicationContext(),definitionSimpleInstallment.class);
    ...
    public void onStart() {
        ...
        Action viewAction = Action.newAction(
                ...
                "SimpleInstallement Page", // TODO: Define a title for the content shown.
